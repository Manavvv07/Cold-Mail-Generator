...
async def test_playwright_job_loader():
    print("Testing Playwright URL Loader with a job posting...")
    
    # Using a GitHub job posting which should be accessible
...
async def main():
    print("Running Playwright job loader test...")
    success = await test_playwright_job_loader()
    
    if success:
        print("\nPlaywright job loader test completed successfully!")
        return True
    else:
        print("\nPlaywright job loader test failed!")
        return False
